...
public partial class MainWindow
{
    ...
    private enum SimpleTab { Play, Servers, Leaderboards, Console, Mods, Notes, Credits, Settings }
    ...
    private void ApplySimpleTab(SimpleTab tab)
    {
        ...
        {
            var showRefresh = tab is SimpleTab.Servers or SimpleTab.Leaderboards;
            BtnBrowserRefresh.Visibility = showRefresh ? Visibility.Visible : Visibility.Collapsed;
            BtnBrowserRefresh.ToolTip = tab == SimpleTab.Leaderboards
                ? Loc.Get("tip_refresh_lb")
                : Loc.Get("tip_refresh_servers");
        }
    ...
    private async Task RefreshServerListAsync(bool quiet)
    {
        ...
        SetBrowserStatus(n == 0
            ...
            : $"{n} server{(n == 1 ? "" : "s")}  ·  {players} player{(players == 1 ? "" : "s")}");
    }
