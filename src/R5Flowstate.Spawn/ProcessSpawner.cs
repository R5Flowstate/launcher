...
public static class ProcessSpawner
{
    ...
    // UI paths; one enumeration covers a whole tick plus the marshal back.
    private const int LivenessMemoMs = 4000;
    ...
    {
        var peek = PeekRoles(installRoot);
        return role == LaunchRole.Client ? peek.client : peek.dedi;
    }

    /// <summary>One process-table walk for both roles. Safe to call off the UI thread.</summary>
    public static (bool client, bool dedi) PeekRoles(string? installRoot = null)
    {
        var trackedClient = GetTrackedPids(LaunchRole.Client).Count > 0;
        var trackedDedi = GetTrackedPids(LaunchRole.Dedicated).Count > 0;
        if (trackedClient && trackedDedi)
            return (true, true);

        if (ReadLivenessMemo(installRoot) is { } hit)
            return (trackedClient || hit.client, trackedDedi || hit.dedi);

        var scan = ScanLiveness(installRoot);
        return (trackedClient || scan.client, trackedDedi || scan.dedi);
    }
