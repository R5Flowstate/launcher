...
public sealed class HostReadyGate : IDisposable
{
    ...
    public static string? SelfCheck()
    {
        ...
        if (ClientErrorDialogText("Script(U):UICodeCallback_ErrorDialog: Connection to server timed out (code:net). See ea.com/unable-to-connect for additional information")
            != "Connection to server timed out (code:net).")
            return "client error dialog text missed";
        if (ClientErrorDialogText("Script(U):UICodeCallback_LevelLoadingFinished:  (true)") is not null)
            return "client error dialog false positive";
        if (!IsHostShutdownLine("Native(E):CHostState::FrameUpdate: Shutdown host game"))
            return "host shutdown missed";
